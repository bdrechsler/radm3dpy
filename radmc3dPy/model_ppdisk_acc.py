...
Generic protoplanetary disk model with a simple chemistry and viscous accretion
    ...
    Viscous accretional heating is implemented in the following way:
    Grid cells touching the mid-plane (i.e. for which one of the cell interfaces is pi/2. in the second
    spherical coordinate) are also radiation sources in the thermal Monte Carlo simulations. They emit
    as a blackbody at the local effective temperature of a viscous accretion disk (e.g. Lynden-Bell & Pringle 1974).

                 3*G*Mstar*Mdot     /                   \^0.5
    Teff(r)^4 = ---------------- * |  1 - (Rstar/r)^0.5 |
                 8*pi*r^3*sigma     \                   /

    The viscous heating is implemented as a continuous starlike source (see the radmc-3D manual for the properties
    of this type of radiation source).
...
from radmc3dPy.analyze import readData, radmc3dRadSources
import sys
...
def getDefaultParams():
    ...
    defpar = [ 
    #['rstar', '[2.0*rs]', 'Radius of the star', 'Radiation sources'],
    ...
    ['nx', '[30,30,50]', 'Number of grid points in the first dimension'],
    ['xbound', '[2.0*rstar[0],1.0*au,1.05*au, 100.0*au]', 'Number of radial grid points'],
    ...
    ['incl_accretion', 'True', ' Switch viscous accretional heating on/off ', 'Radiation sources'],
    ['accrate', '1e-6*ms/year', ' Accretion rate ', 'Radiation sources'],
    ['starsurffrac', '0.1', ' Fraction of the stellar surface covered by the hot spot/boundary layer', 'Radiation sources']]
...
def getStellarsrcDensity(grid=None, ppar=None):
    ...
    INPUT:
    ------
        grid - An instance of the radmc3dGrid class containing the spatial and frequency grid
        ppar - A dictionary containing the parameters of the model 
    ...
    src  = radmc3dRadSources(ppar=ppar, grid=grid)
...
def getStellarsrcTemplates(grid=None, ppar=None):
    ...
    dum = zeros([src.cststar.shape[0], 3], dtype=float) 
    #if ppar['accrate']==0.:
        #dum[:,0] = 0.
    #else:
    dum[:,0] = src.cststar
